...
public class PotentialFields {
	...
	public PotentialFields() {
		...
		// TODO: Add a button for fractional progress.
		fractionalProgressID = gui.addButton(7, 2, "Fractional", this, "enableFractional" );
...
//		
	...
	public void enableFractional(){
		setMoveType(true, false, false);
	}
	
	public void enableArc() {
		setMoveType(false, true, false);
	}
	
	public void disableArc() {
		setMoveType(false, false, true);
	}

	private void setMoveType(boolean fractional, boolean arcMove, boolean defMove)
	{
		this.fractional = fractional;
		this.arcMove = arcMove;
		this.defMove = defMove;
		gui.setButtonEnabled(fractionalProgressID, !fractional);
		gui.setButtonEnabled(enArcId, !arcMove);
		gui.setButtonEnabled(disArcId, !defMove);
	}
...
//                
	...
	public void goLittleRobot(IntPoint start, IntPoint goal, int goalRad, int robotRadius, int robotSensorRange,
		...
		while (!rob.inGoal()) {
			...
			boolean move = false;
			if(arcMove) move = rob.ArcMove();
			if(defMove) move = rob.move();
			if(fractional) move = rob.fractional();
